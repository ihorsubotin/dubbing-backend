...
export class SubtitlesService extends GenericCrudService<SubtitleEntry> {
	...
	async generate(generateSubtitlesDto: GenerateSubtitlesDto): Promise<number> {
		if (
			...
			let audios = generateSubtitlesDto.forAudio.map((forAudio) =>
	...
	async translate(translateSubtitlesDto: TranslateSubtitlesDto) {
		...
		if (
			...
			let audios = translateSubtitlesDto.forAudio.map((forAudio) =>
				this.audioFilesService.findOne(forAudio),
			) as AudioFile[];
			audios = audios.filter((audio) => audio !== undefined);
			for (const audio of audios) {
				const subtitles = this.search(audio.id);
				oldSubtitles = subtitles.filter(
					(subtitle) => subtitle.language === translateSubtitlesDto.language,
				);
				translateFrom = subtitles.filter(
					(subtitle) => subtitle.translationOf === undefined,
				);
			}
		} else {
			if (
				...
				const subtitles = translateSubtitlesDto?.forSubtitle.map((id) =>
					this.findOne(id),
				) as SubtitleEntry[];
				for (const subtitle of subtitles) {
					oldSubtitles.push(
						...this.findAll().filter(
							(entry) => entry.translationOf === subtitle.id,
						),
					);
				}
				translateFrom = subtitles.filter(
					(subtitle) =>
						subtitle !== undefined && subtitle.translationOf === undefined,
				);
			} else {
	...
	getOriginalByTranslation(subtitles: SubtitleEntry[]) {
		const originals = subtitles
			.map((subtitle) =>
				subtitle.translationOf
					? this.findOne(subtitle.translationOf)
					: undefined,
			)
	...
	search(
		audioId: number | undefined = undefined,
	...
	async remove(id: number) {
		...
		if (subtitle) {
			const translations = this.findAll().filter(
				(entry) => entry.translationOf === subtitle.id,
			);
